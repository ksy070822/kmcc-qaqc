...
async function runSync() {
  ...
  try {
    ...
    for (let i = 0; i < evaluationIds.length; i += CHUNK_SIZE) {
      ...
      const idsList = chunk.map(id => `'${String(id).replace(/'/g, "''")}'`).join(',');
  ...
  const bigqueryRows = newEvaluations.map((evalData) => {
    ...
    // Assuming group = service or service + ' ' + channel if uncertain. 
    // The schema description says "그룹 (서비스/채널 조합)". 
    ...
    return {
      ...
      consultation_datetime: consultDate ? new Date(consultDate).toISOString() : null, // TIMESTAMP expects valid format
