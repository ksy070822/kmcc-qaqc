...
export function ChatDashboard({ overview, verticalStats, processingTime, weeklySummary, prevOverview, prevVerticalStats, prevProcessingTime, month }: Props) {
  const target = PRODUCTIVITY_TARGETS.chat
  ...
  // 주간 요약 — 센터 합산
  const weeklyRows = (() => {
    const weekMap = new Map<string, { weekLabel: string; weekStart: string; incoming: number; answered: number; dayCount: number }>()
    for (const row of weeklySummary) {
      const entry = weekMap.get(row.weekStart) || { weekLabel: row.weekLabel, weekStart: row.weekStart, incoming: 0, answered: 0, dayCount: 0 }
  ...
  return (
    <div className="space-y-4">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-xs text-right">인입</TableHead>
                <TableHead className="text-xs text-right">응답</TableHead>
                <TableHead className="text-xs text-right">TPD</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
      ...
      {/* 채팅 처리시간 — 대기시간 목표(2분) + 처리시간 목표 */}
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">채팅 처리시간 분석 ({month})</CardTitle>
        </CardHeader>
        <CardContent className="px-4 pb-4">
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-xs text-right">
                  대기시간
                  <span className="text-[10px] text-muted-foreground ml-1">(목표 {fmtSec(target.waitTime)})</span>
                </TableHead>
                <TableHead className="text-xs text-right">채팅시간</TableHead>
                <TableHead className="text-xs text-right">후처리</TableHead>
            ...
            <TableBody>
              {processingTime.length === 0 ? (
                ...
                [...processingTime].sort(sortCenterVertical).map((row, i) => {
                  const waitExceeded = row.avgWaitTime > target.waitTime
                  const targetSec = target.handling[row.vertical] ?? null
                  const ahtMet = targetSec != null ? isTargetMet(row.avgHandlingTime, targetSec, "lower") : null
                  return (
                    <TableRow key={i}>
                      <TableCell className="text-xs">{row.center}</TableCell>
                      <TableCell className="text-xs">{row.vertical}</TableCell>
                      <TableCell className={`text-xs text-right font-medium ${waitExceeded ? "text-red-500" : ""}`}>
                        {fmtSec(row.avgWaitTime)}
                      </TableCell>
                      ...
                      <TableCell className={`text-xs text-right font-medium ${ahtMet === false ? "text-red-500" : ""}`}>
                        {fmtSec(row.avgHandlingTime)}
                      </TableCell>
                      <TableCell className="text-xs text-right">
                        {targetSec != null ? (
                          <span className={ahtMet ? "text-green-600" : "text-red-500"}>
                            {fmtSec(targetSec)} {ahtMet ? "●" : "○"}
                          </span>
                        ) : (
